...
var __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$navigation$2e$js__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/node_modules/next/navigation.js [app-ssr] (ecmascript)"); // Import จาก next/navigation ใน App Router
"use client"; // จำเป็นต้องใช้ client component เพราะมี state และ event handling
;
;
;
function LoginPage() {
    ...
    const [isLoading, setIsLoading] = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$ssr$2f$react$2e$js__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["useState"])(false); // เพิ่ม state สำหรับ loading
    const router = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$navigation$2e$js__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["useRouter"])(); // เรียกใช้ useRouter hook
    // ทำให้เป็น async function เพื่อรอ fetch
    const handleLogin = async (event)=>{
        event.preventDefault(); // ป้องกันการ submit form แบบปกติ
        setError(''); // เคลียร์ error เก่า
        setIsLoading(true); // เริ่ม loading
        try {
            // --- 1. ส่ง Request ไปยัง API Route ---
            ...
            // --- 2. จัดการ Response จาก API ---
            const data = await response.json(); // แปลง response กลับเป็น object
            if (response.ok && data.success) {
                console.log('Login successful (from API):', data.message);
                // (ตัวเลือกเสริม) อาจจะเก็บ token ที่ได้จาก data.token ไว้ใน state, context หรือ local storage สำหรับใช้งานต่อไป
                // localStorage.setItem('authToken', data.token);
                // Redirect ไปหน้า Admin
                router.push('/admin'); // ไปยัง Route /admin ซึ่งจะโหลด src/app/admin/page.js
            } else {
                // ถ้า API ตอบกลับมาว่าไม่สำเร็จ (เช่น status 401 หรือ success: false)
                setError(data.message || 'ชื่อผู้ใช้หรือรหัสผ่านไม่ถูกต้อง'); // แสดงข้อความ error จาก API
                console.error('Login failed (from API):', data.message);
            }
        } catch (err) {
            // --- 3. จัดการ Error กรณี Network หรือ Fetch ล้มเหลว ---
            console.error('API Fetch error:', err);
            setError('เกิดข้อผิดพลาดในการเชื่อมต่อ ไม่สามารถติดต่อเซิร์ฟเวอร์ได้');
        } finally{
            setIsLoading(false); // หยุด loading ไม่ว่าผลจะเป็นอย่างไร
        }
    };
                        ...
                        /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$ssr$2f$react$2d$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                            ...
                            children: [
                                ...
                                /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$ssr$2f$react$2d$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["jsxDEV"])("input", {
                                    type: "password",
                                    id: "password",
                                    // *** สำคัญ: รหัสผ่านใน API คือ "1234" ไม่ใช่ "password123" ***
                                    // ผู้ใช้ต้องกรอก "1234" เพื่อให้ Login ผ่าน API นี้ได้
                                    value: password,
...
//# sourceMappingURL=%5Broot-of-the-server%5D__494a146e._.js.map